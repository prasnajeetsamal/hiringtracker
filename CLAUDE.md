...
- **Gmail SMTP transport** - `lib/email.js` is now transport-agnostic. When `GMAIL_USER` + `GMAIL_APP_PASSWORD` are set, sends route through Gmail's SMTP server via `nodemailer` (no DNS work required). Falls back to Resend, then to no-op. `email_log.status` is suffixed with the transport (`sent[smtp]` / `sent[resend]`). Shipped.
- **Activity heatmap** - new `ActivityHeatmap` component on `/reports` showing daily candidates added / advanced / rejected over the last 60 days as a stacked column strip. Included in the chart picker (PDF / HTML / Print export), in scheduled reports (cron + `ScheduledReportsPage` section list), and in the on-screen Reports view. Shipped.
- **Per-role insights** - `RoleDetailPage` gained an Insights card above the Pipeline board: 4 KPIs (Active / Hired / Rejected / Median time-to-hire), an active-by-stage mini funnel, top 5 scorers on this role, and stale candidates (7+ days idle). "Full report" link jumps to `/reports` with this role's filter pre-applied. Shipped.
- **Bulk candidate actions** - `/candidates` table has per-row + select-all checkboxes. When any row is selected, a sticky indigo toolbar surfaces with **Advance stage** / **Reject** / **Add tag** actions. Operations run in parallel batches of 4 via the existing `transitionCandidate` endpoint (or a direct `tags` upsert for the tag action). Toast shows live progress (`Advancing (3 / 12)...`) and a final success/failure count. Shipped.
- **Saved searches** - `/candidates` filter bar has a **Save filter** pill that prompts for a name; saved combinations show as indigo pills above the table (click to re-apply, X to delete). Stored in `localStorage` under `slate.candidates.saved-searches` (per browser; no DB schema added). Caps at 12 entries. Shipped.
- **Candidate self-serve status page** - new PUBLIC route `/c/:token` (no Slate login required) where the candidate can see their pipeline status. Migration `0011` adds `candidates.public_token uuid unique` (auto-populated on insert). `api/public-status` returns a narrow safe-view (first name, role/project, stage timeline, what-to-expect) - explicitly hides AI scores, feedback, comments, interviewer names. Mounted in `src/App.jsx` OUTSIDE the auth `Gate` so it bypasses sign-in entirely. CandidateDetailPage sidebar gained a "Candidate status link" card with copy-to-clipboard. Shipped.
- **Mobile-first responsive pass** - audit-driven CSS tightening across Dashboard, Candidates list, Candidate detail, AppShell, PageHeader, FilterBar. `PageHeader` actions stack below title on mobile (`flex-col sm:flex-row`). AppShell drops main padding to `p-4 sm:p-6`. Candidates table hides low-priority columns under `md` (Role/Project, Source) and under `sm` (Status, AI, arrow), surfacing role inline under the candidate name on the smallest screens. KPI numbers + AI score scale via `text-2xl sm:text-3xl`. Pipeline funnel meta strip, Pipeline legend, Status-link input, AI evaluation header, SeedDataCallout buttons all wrap or stack cleanly. Single codebase - no parallel mobile component tree. Shipped.
- **HTML pitch deck** - `pitch/Slate Pitch Deck.html` is a 1920x1080 marketing deck (8 slides: hero, pitch, features, pipeline, AI, reports, candidate experience, CTA) using the same `deck-stage.js` runtime as the Stats Engine reference. Slate palette (indigo/violet/pink). Screenshots referenced via `pitch/screens/<name>.png`; placeholders show until real files are dropped in. See `pitch/README.md` for the capture workflow. Shipped.
...
| `0011_candidate_public_token.sql` | Adds `candidates.public_token uuid unique not null default gen_random_uuid()` (backfilled for existing rows). Powers the public `/c/:token` self-serve status page via `api/public-status`. No RLS policy changes - the endpoint uses the service-role client and returns only a narrow safe-view subset, so direct browser reads still require the existing membership policies. |
...
| `api/public-status` | **No-auth PUBLIC endpoint.** GET `?token=<uuid>` (or POST with `{token}`) returns a narrow safe-view of one candidate's status: first name, role title, project name, stage timeline with `what_to_expect` per stage, and overall `status`. Token possession IS the auth - the service-role client reads `candidates.public_token = ?`. Explicitly excludes AI scores, feedback, comments, interviewer names, contact details. Powers the public `/c/:token` page in `src/App.jsx` (mounted outside the auth Gate). |
